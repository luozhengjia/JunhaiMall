...
public class CommonUtil {
	...
	public static String getTemplateUrl(HttpServletRequest request){
		...
		return "template_"+version;
	}
	
	public static <T> List<T> getCurrentPageData(int startIndex, int pageSize,List<T> secList){
		if(secList == null || secList.size() <= 0){
			return new ArrayList<T>(0);
		}
		Integer totalCount = secList.size();
		if(startIndex >= totalCount){
			return new ArrayList<T>(0);
		}
		Integer toIndex = (startIndex + pageSize) > totalCount ? totalCount : (startIndex + pageSize);
		return secList.subList(startIndex, toIndex);
	}
}
