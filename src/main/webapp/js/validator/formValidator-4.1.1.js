...
$.formValidator = 
{
	...
	showMessage : function(returnObj)
	{
		...
		{		
			...
			{
				$.formValidator.setTipState(elem,showclass,' ');
				if(showmsg!=" ")
				{
				$("#"+id+"FixTip").css('color','red');
				$("#"+id+"FixTip").html(showmsg);
				}
				else
				{
					$("#"+id+"FixTip div").css('color','red');
				}
			}
		}
		else
...
$.fn.formValidator = function(cs) 
{
	...
	return this.each(function(e)
	{
		...
		if(fixTip.length==1 && onMouseOutFixTextHtml!="" && onMouseOnFixTextHtml!="" && showFixText != "")
		{
			...
			//	}
			//);
			//fixTip.css("padding","4px 0px 10px 0px").css("margin","0px 0px 0px 0px").html(onMouseOutFixTextHtml.replace(/\$data\$/g, setting.onShowFixText)); 
			fixTip.addClass("tipinfo").html(onMouseOutFixTextHtml.replace(/\$data\$/g, setting.onShowFixText)); 
			//fixTip.parent().parent().find("th div").css("padding","4px 0px 10px 0px").css("margin","0px 0px 0px 0px");
			fixTip.parent().parent().find("th div").addClass("tipinfo");
			//fixTip.parent().find("th").css("padding","5px 0px 5px 0px").css("margin","0px 0px 0px 0px");
		}
		//获取输入框内的提示内容
