(function() {
	...
	JunHai.Biz = {
		login : function(){
			...
			if(JunHai.Util.isEmpty(couponPassword)){
				alert("请输入优惠券密码");
				return;
			}
			
			if(JunHai.Util.isEmpty(validateCode)){
				alert("请输入验证码");
				return;
			}
			$.ajax({
			   	...
			   	data : {
			   		...
			   		"couponPassword" : hex_md5(couponPassword),
				   	"validateCode" :validateCode
			   	},
			   	success: function(data){
			   		if(!JunHai.Util.isEmpty(data)){
			   			data = eval("(" + data + ")");
			   			if(parseInt(data.state) == 0){
			   				$('#couponPassword').val(hex_md5( $("#couponPassword").val()));
			   				$("#couponForm").submit();
			   			}else{
			   				alert(data.msg);
			   			}
